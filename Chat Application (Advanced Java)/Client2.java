...
class Client2
{
	...
	throws Exception
	{
		...
		s=new Socket("localhost",1088);//create client socket
		
		//dos=new DataOutputStream(s.getOutputStream());//to send data to the server
		br=new BufferedReader(new InputStreamReader(s.getInputStream()));//to read data coming from server
	   // kb=new BufferedReader(new InputStreamReader(System.in));//to read data from keyboard 
	...
	static void dis()
	{
			...
			f=new JFrame("Chat Application2");
			f.setSize(500,500);
			...
			JTextField tf1=new JTextField("Nahush Bhade",10);
			JLabel jl1=new JLabel("Contact No:");
			JTextField tf2=new JTextField("9766874251",10);
	...
	static void settext(String str1)
	{
		ta.setText(str1);
		//settext();
	}
